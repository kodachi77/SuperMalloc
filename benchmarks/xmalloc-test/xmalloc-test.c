...
#define xmalloc super_malloc
#define xfree   super_free
...
double
elapsed_time( struct timeval* time0 )
{
...
static const long possible_sizes[] = {
    8, 12, 16, 24, 32, 48, 64, 96, 128, 192, 256, ( 256 * 3 ) / 2, 512, ( 512 * 3 ) / 2, 1024, ( 1024 * 3 ) / 2, 2048 };
...
void
enqueue_batch( struct batch* batch )
{
    pthread_mutex_lock( &lock );
    while( batch_count >= batch_count_limit && !atomic_load( &done_flag ) ) { pthread_cond_wait( &full_cv, &lock ); }
...
{
    pthread_mutex_lock( &lock );
    while( batches == NULL && !atomic_load( &done_flag ) ) { pthread_cond_wait( &empty_cv, &lock ); }
...
mem_releaser( void* arg )
{
    ...
    while( !atomic_load( &done_flag ) )
    {
        ...
        if( b )
        {
            for( int i = 0; i < OBJECTS_PER_BATCH; i++ ) { xfree( b->objects[i] ); }
            xfree( b );
        }
...
run_memory_free_test()
{
    ...
    /* Initialize counter */
    for( i = 0; i < num_workers; ++i ) counters[i].c = 0;
    ...
    for( i = 0; i < num_workers * 2; ++i ) pthread_join( thread_ids[i], &ptr );
...
main( int argc, char** argv )
{
    ...
    while( ( c = getopt( argc, argv, "w:t:ds:v" ) ) != -1 )
    {
        switch( c )
        {
            ...
            case 's': object_size = atoi( optarg ); break;
