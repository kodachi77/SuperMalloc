...
REALLOC( void* p, size_t size )
{
    ...
    if( oldsize < size )
    {
        ...
        for( size_t i = 0; i < oldsize; i++ ) { ( (char*) result )[i] = ( (char*) p )[i]; }
        FREE( p );
        return result;
    }
    if( oldsize > 16 && size < oldsize / 2 )
    {
        void* result = MALLOC( size );
        if( !result ) return NULL;    // without disrupting the contents of p.
        for( size_t i = 0; i < size; i++ ) { ( (char*) result )[i] = ( (char*) p )[i]; }
        return result;
    }
    return p;
}
...
void
test_malloc_usable_size( void )
{
    for( size_t i = 8; i < 4 * chunksize; i *= 2 )
    {
        for( size_t o = 0; o < 8; o++ ) { test_malloc_usable_size_internal( i + o ); }
    }
}
#endif

void*
object_base( void* ptr )
{
