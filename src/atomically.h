...
class mylock_raii
{
   ...
   public:
    mylock_raii( lock_t* mylock ) : mylock( mylock )
    {
        switch( mode )
        {
            case MODE_PTHREAD_MUTEX: pthread_mutex_lock( &mylock->pt_m ); break;
            case MODE_TSX: futex_mutex_lock( &mylock->f_m ); break;
            default: abort();
        }
    }
    ~mylock_raii()
    {
        switch( mode )
        {
            case MODE_PTHREAD_MUTEX: pthread_mutex_unlock( &mylock->pt_m ); break;
            case MODE_TSX: futex_mutex_unlock( &mylock->f_m ); break;
            default: abort();
        }
    }
};
...
atomically( lock_t* mylock, const char* name __attribute__( ( unused ) ), void ( *predo )( Arguments... args ),
...
template <typename ReturnType, typename... Arguments>
static inline ReturnType
atomically2( lock_t* lock0, lock_t* lock1, const char* name __attribute__( ( unused ) ), void ( *predo )( Arguments... args ),
             ReturnType ( *fun )( Arguments... args ), Arguments... args )
{
    if( mode == MODE_PTHREAD_MUTEX )
    {
