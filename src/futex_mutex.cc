...
futex_mutex_unlock( futex_mutex_t* m )
{
    ...
    else
    {
        while( 1 )
        {
            ...
            if( old_hold ) { futex_wakeN( &m->hold ); }
            else
            {
                break;
            }
...
extern "C" int
futex_mutex_subscribe( futex_mutex_t* m )
{
    return atomic_load( &m->lock ) & 1;
}

extern "C" int
futex_mutex_hold( futex_mutex_t* m )
{
    //set_threadid();
...
static bool
time_less( const struct timespec& a, const struct timespec& b )
{
...
stress()
{
    ...
    start.tv_sec++;
    uint64_t locked_fast = 0, locked_slow = 0, sub_locked = 0, sub_unlocked = 0, wait_long = 0, wait_short = 0, wait_was_one = 0,
             wait_was_zero = 0;
    while( 1 )
    {
        ...
        for( uint64_t i = 0; i < 100; i++ )
        {
            switch( i % 3 )
            {
                ...
                case 1:
                    if( futex_mutex_subscribe( &m ) ) { sub_locked++; }
                    else
                    {
                        sub_unlocked++;
                    }
                    break;
                case 2:
                    if( futex_mutex_hold( &m ) ) { wait_long++; }
                    else
                    {
                        wait_short++;
                    }
                    if( futex_mutex_subscribe( &m ) ) { wait_was_one++; }
                    else
                    {
                        wait_was_zero++;
                    }
    ...
    printf(
        "locked_fast=%8ld locked_slow=%8ld sub_locked=%8ld sub_unlocked=%8ld wait_long=%8ld wait_short=%8ld was1=%8ld was0=%ld\n",
...
stress_test()
{
    const int   n = 8;
    std::thread x[n];
    for( int i = 0; i < n; i++ ) { x[i] = std::thread( stress ); }
