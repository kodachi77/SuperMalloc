...
static void
put_cached_power_of_two_chunks( chunknumber_t cn, int list_number )
{
    ...
    else
    {
        ...
        while( 1 )
        {
            ...
            if( atomic_compare_and_swap( (volatile _Atomic( uint32_t )*) &free_chunks[list_number], &hd, cn ) )
                break;    //> added &
        }
    }
}
...
static void*
get_power_of_two_n_chunks( chunknumber_t n_chunks )
